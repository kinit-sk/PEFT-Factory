...
def discover_custom_peft_methods(peft_dir: str) -> dict[str, tuple[type[PeftConfig], type[BaseTuner]]]:
    ...
    if not peft_path.exists():
        logger.warning(f"PEFT directory not found: {peft_dir}")
        return discovered_methods

    if not peft_path.is_dir():
        logger.warning(f"PEFT path is not a directory: {peft_dir}")
        return discovered_methods
...
def _load_peft_method(
    ...
    if config_file.exists():
        try:
            ...
            logger.warning(f"Failed to load config for {method_name}: {e}")
    else:
        logger.warning(f"No config.py found in {method_dir}")
        return None, None
    ...
    if model_file.exists():
        try:
            ...
            logger.warning(f"Failed to load model for {method_name}: {e}")
    else:
        logger.warning(f"No model.py found in {method_dir}")
        return None, None
...
def _find_peft_config_class(module, method_name: str) -> Optional[type[PeftConfig]]:
    ...
    logger.warning(f"No valid PeftConfig subclass found in {method_name} config module")
    return None
...
def _find_base_tuner_class(module, method_name: str) -> Optional[type[BaseTuner]]:
    ...
    logger.warning(f"No valid BaseTuner subclass found in {method_name} model module")
    return None
